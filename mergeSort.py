...
class MergeSort(SorterUI):
    def __init__(self, merge=None):
        super().__init__(merge)
        self.window.title("MERGE SORT ALGORITHM")
    ...
    def merge_sort(self, array_data, left, right):
        if left < right:
            mid = (left + right) // 2
            self.merge_sort(array_data, left, mid)
            self.merge_sort(array_data, mid + 1, right)
            self.merge(array_data, left, mid, right)

    def merge(self, array_data, left, mid, right):
        speed = self.set_sort_speed()
        left_array = array_data[left:mid + 1]
        right_array = array_data[mid + 1:right + 1]
        ...
        while i < len(left_array) and j < len(right_array):
            ...
            if left_array[i] <= right_array[j]:
                array_data[k] = left_array[i]
                i += 1
            else:
                array_data[k] = right_array[j]
                j += 1

            color_array = [YELLOW if x >= left and x <= k else BLUE for x in range(len(array_data))]
        ...
        while i < len(left_array):
            ...
            color_array = [PURPLE if x >= left and x <= k else BLUE for x in range(len(array_data))]
            self.array_data_draw(array_data, color_array)
            self.window.update()
            self.window.after(int(speed * 500))
    ...
    def run_sort(self):
        ...
        self.merge_sort(array_data, 0, len(array_data) - 1)

        self.array_data_draw(array_data, [DARK_BLUE for x in range(len(array_data))])
...
if __name__ == "__main__":
    window = Tk()
    merge_frame = MergeSort(window)
    merge_frame.pack()

    sorting_thread = threading.Thread(target=merge_frame.run_sort)
