...
class SelectionSort(SorterUI):
    def __init__(self, selection=None):
        super().__init__(selection)
        self.window.title("SELECTION SORT ALGORITHM")
    ...
    def run_sort(self):
        ...
        for i in range(n - 1):
            ...
            for j in range(i + 1, n):
                ...
                if array_data[j] < array_data[min_index]:
                    min_index = j

                color_array = [YELLOW if x == min_index or x == j else BLUE for x in range(len(array_data))]
                self.array_data_draw(array_data, color_array)
                self.window.update()
                self.window.after(int(speed * 500))
                print("sort speed: ", speed)

            array_data[i], array_data[min_index] = array_data[min_index], array_data[i]
            color_array = [YELLOW if x == i or x == min_index else BLUE for x in range(len(array_data))]
            self.array_data_draw(array_data, color_array)
            self.window.update()
            self.window.after(int(speed * 500))
            print("sort speed: ", speed)
...
if __name__ == "__main__":
    window = Tk()
    selection_frame = SelectionSort(window)
    selection_frame.pack()

    sorting_thread = threading.Thread(target=selection_frame.run_sort)
